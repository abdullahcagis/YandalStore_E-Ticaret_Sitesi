...
using ABDUBank.Models;

namespace ABDUBank.Controllers.API
{
    ...
    {
        ABDUBANK_DBEntities db = new ABDUBANK_DBEntities();
        ...
        public IHttpActionResult PostPayService(string MerchantCode, string MerchantPassword, string CardNo, string EXPM, string EXPY, string CVV, decimal Price)
        {
            ...
            if (merchantCount != 0)
            {
                ...
                int CardControl = db.MusteriHesap.Count(x => x.KartNumarası == CardNo);
                if (CardControl != 0)
                {
                    MusteriHesap mh = db.MusteriHesap.FirstOrDefault(x => x.KartNumarası == CardNo);
        ...
        public IHttpActionResult PostPayService(PayServiceModel model)
        {
            ...
            if (merchantCount != 0)
            {
                ...
                int cardControl = db.MusteriHesap.Count(x => x.KartNumarası == model.CardNo);
                if (cardControl != 0)
                {
                    MusteriHesap mh = db.MusteriHesap.FirstOrDefault(x => x.KartNumarası == model.CardNo);
